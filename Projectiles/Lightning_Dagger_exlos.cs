...
namespace Infernus.Projectiles
{
    ...
    public class Lighning_Dagger_exlos : ModProjectile
    {
        ...
        public override void OnSpawn(IEntitySource source)
        {
            ...
            for (int k = 0; k < 14; k++)
            {
                ...
                Dust Sword = Dust.NewDustPerfect(Projectile.Center + speed2 * 32, DustID.Electric, speed2 * 3, Scale: 2f);
                Sword.noGravity = true;
            }
            for (int k = 0; k < 12; k++)
            {
                Vector2 speed2 = Main.rand.NextVector2Circular(2f, 4f);
                Dust Sword = Dust.NewDustPerfect(Projectile.Center + speed2 * 32, DustID.UltraBrightTorch, speed2 * 3, Scale: 2f);
                Sword.noGravity = true;
            }
            for (int k = 0; k < 6; k++)
            {
